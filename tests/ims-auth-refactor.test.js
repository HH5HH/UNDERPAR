...
test("selected media company refresh starts direct premium hydration immediately and keeps CM matching bounded", () => {
  ...
  assert.ok(
    refreshPanelsSource.indexOf("renderPremiumServicesLoading(programmer, { controllerReason });") <
      refreshPanelsSource.indexOf("let resolvedServices = await premiumHydrationPromise;")
  );
...
test("esm health reuses the hydrated ESM premium service auth context instead of a console tenant-token helper", () => {
  ...
  assert.match(resolveAuthSource, /const appInfo = resolveProgrammerPremiumServiceRuntimeApp\("esm",\s*programmerId,\s*services\) \|\| null;/);
  assert.doesNotMatch(resolveAuthSource, /selectPreferredEsmAppForRequestor\(/);
...
test("media company selection uses cached live AdobePass apps or direct applications queries without selection-time vault hydration", () => {
  ...
  assert.match(refreshPanelsSource, /renderPremiumServicesLoading\(programmer,\s*\{\s*controllerReason\s*\}\);/);
  assert.match(refreshPanelsSource, /const persistedFinalRecord = await persistPassVaultProgrammerRecord\(programmer,\s*finalServices,\s*\{/);
  assert.match(refreshPanelsSource, /applyPassVaultRecordToRuntime\(programmer,\s*persistedFinalRecord,\s*\{/);
...
test("premium app details still retain software statements while pass-vault mapping stays scope-driven", () => {
  ...
  assert.match(sanitizeApplicationSource, /const fetchOrder = Number\(source\?\.__underparFetchOrder\);/);
  assert.match(sanitizeApplicationSource, /sanitized\.__underparFetchOrder = fetchOrder;/);
...
test("premium UI waits for reusable programmer vault state while runtime readiness stays DCR-backed and pass-vault compilation persists before CM background hydration", () => {
  ...
  assert.match(compileSource, /const persistedRecord = await persistPassVaultProgrammerRecord\(programmer,\s*mergedServices,\s*\{/);
  assert.match(compileSource, /applyPassVaultRecordToRuntime\(programmer,\s*persistedRecord,\s*\{/);
  assert.match(compileSource, /void ensureCmHydratedForProgrammer\(programmer,\s*persistedServices,\s*\{/);
...
test("pass vault compilation uses LoginButton-style registered-app ordering, resolves selected service apps, and hydrates only the selected service apps", () => {
  ...
  const fetchApplicationsSource = extractFunctionSource(popupSource, "fetchApplicationsForProgrammer");
  const orderedCandidatesSource = extractFunctionSource(popupSource, "buildOrderedPremiumServiceCandidates");
  const restV2CandidateSource = extractFunctionSource(popupSource, "collectRestV2AppCandidatesFromPremiumApps");
  const passVaultServiceGuidsSource = extractFunctionSource(popupSource, "getPassVaultServiceAppGuidsFromRecord");
  const runtimeAppResolutionSource = extractFunctionSource(popupSource, "resolveProgrammerPremiumServiceRuntimeApp");
  ...
  assert.match(runtimeAppResolutionSource, /const preferredMatch =[\s\S]*selectPreferredPassVaultHydrationServiceApplication\("restV2", candidates, normalizedProgrammerId\)/);
  ...
  assert.match(
    directServicesSource,
    /esm:\s*hydratedServiceEntries\?\.esm\?\.registeredApplication\s*\|\|\s*selectPreferredPassVaultHydrationServiceApplication\("esm",\s*esmApps,\s*programmerId\)\s*\|\|\s*null,/
  );
  assert.match(
    directServicesSource,
    /degradation:\s*hydratedServiceEntries\?\.degradation\?\.registeredApplication\s*\|\|\s*selectPreferredPassVaultHydrationServiceApplication\("degradation",\s*degradationApps,\s*programmerId\)\s*\|\|\s*null,/
  );
  assert.doesNotMatch(popupSource, /function mergeDetectedPassVaultServices\(/);
  assert.doesNotMatch(popupSource, /function resolveMissingPassVaultServiceMappings\(/);
...
test("REST V2 app selection stays media-company scoped and keeps the programmer primary authoritative", () => {
  ...
  assert.match(collectScopedRestV2Source, /buildPassVaultHydrationRegisteredApplications\(runtimeApplications \|\| \{\}\)[\s\S]*collectRestV2AppCandidatesFromPremiumApps\(premiumApps\)/);
  ...
  assert.match(
    ...
    /const preferredMatch =[\s\S]*selectPreferredPassVaultHydrationServiceApplication\("restV2",\s*candidates,\s*normalizedProgrammerId\)[\s\S]*selectPreferredRestV2AppForRequestor\(candidates,\s*"",\s*normalizedProgrammerId\);/
  );
  ...
  assert.doesNotMatch(popupSource, /function orderRestV2AppCandidatesForRequestor\(/);
  assert.doesNotMatch(popupSource, /function fetchRestV2ConfigurationUsingCandidateApps\(/);
...
test("premium detection promotes TempPASS as a first-class premium workspace alongside CM tenant catalog detection", () => {
  ...
  assert.match(refreshPanelsSource, /const persistedFinalRecord = await persistPassVaultProgrammerRecord\(programmer,\s*finalServices,\s*\{/);
  assert.match(refreshPanelsSource, /renderPremiumServices\(renderServices,\s*programmer,\s*\{\s*controllerReason\s*\}\);/);
...
test("premium panel rendering waits for persisted programmer vault state and keeps workspace hydration tied to runtime readiness", () => {
  const popupSource = fs.readFileSync(path.join(ROOT, "popup.js"), "utf8");
  const primeSource = extractFunctionSource(popupSource, "primeProgrammerServiceHydration");
  const refreshSource = extractFunctionSource(popupSource, "refreshProgrammerPanels");
  const compileSource = extractFunctionSource(popupSource, "queuePassVaultProgrammerCompilation");
...
test("available TempPASS credentials participate in programmer reuse readiness and successful hydration clears stale top-level status", () => {
  ...
  assert.match(
    refreshSource,
    /clearProgrammerPremiumHydrationProgress\(programmerId\);[\s\S]*setProgrammerWorkspaceHydrationReady\(programmerId,\s*hrContextReady\);[\s\S]*clearStatusUnlessCmTenantsPrecheckBlocked\(\);[\s\S]*renderPremiumServices\(renderServices,/
  );
});
